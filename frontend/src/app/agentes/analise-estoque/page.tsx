...
export default function AnaliseEstoquePage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="text-2xl font-semibold text-slate-900">Análise de Estoque</h1>
      ...
      {resultado && (
        <div className="flex flex-col gap-4">
          ...
          <div className="rounded-lg border border-slate-200 bg-white p-5">
            ...
            <p className="text-sm text-slate-600">{resultado.resumo}</p>
          </div>
            ...
            <div className="overflow-x-auto rounded-lg border border-slate-200 bg-white">
              <table className="min-w-full divide-y divide-slate-200 text-sm">
                <thead className="bg-slate-50">
                  <tr>
                    <th className="px-4 py-2 text-left font-medium text-slate-500">Decisão</th>
                    <th className="px-4 py-2 text-left font-medium text-slate-500">Margem resultante</th>
                    <th className="px-4 py-2 text-left font-medium text-slate-500">Justificativa (Financeiro)</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {resultado.decisoes.map((d) => (
                    <tr key={d.precificacao_id}>
                      ...
                      <td className="px-4 py-2 text-slate-600">{d.justificativa}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <p className="text-xs text-slate-400">
            ...
            <a href="/auditoria" className="text-emerald-600 hover:underline">
