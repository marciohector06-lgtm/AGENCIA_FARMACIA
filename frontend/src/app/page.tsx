  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="text-sm text-slate-500">Visão geral da operação e das decisões dos agentes de IA.</p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <div className="rounded-lg border border-slate-200 bg-white p-5">
          <h2 className="mb-3 text-sm font-semibold text-slate-700">Ações rápidas</h2>
          <div className="flex flex-col gap-2">
            <Link href="/agentes/analise-estoque" className="text-sm font-medium text-emerald-600 hover:underline">
              Rodar Análise de Estoque (Gerente → Financeiro)
            </Link>
            <Link href="/atendimento" className="text-sm font-medium text-emerald-600 hover:underline">
              Abrir o Atendimento (Avatar)
            </Link>
            <Link href="/auditoria" className="text-sm font-medium text-emerald-600 hover:underline">
        ...
        <div className="rounded-lg border border-slate-200 bg-white p-5">
          <h2 className="mb-3 text-sm font-semibold text-slate-700">Decisões recentes dos agentes</h2>
          {!kpis && <p className="text-sm text-slate-400">Carregando...</p>}
          {kpis && kpis.logsRecentes.length === 0 && (
            <p className="text-sm text-slate-400">Nenhuma decisão registrada ainda.</p>
          )}
          <ul className="flex flex-col gap-2">
            {kpis?.logsRecentes.map((log) => (
              <li key={log.id} className="border-b border-slate-100 pb-2 text-sm last:border-0">
                <p className="text-slate-700">{log.decisao_tomada}</p>
