...
export function Sidebar({ mobileOpen, onCloseMobile }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-40 flex h-screen w-64 shrink-0 flex-col border-r border-white/[0.06] bg-[#08090e] transition-transform duration-200 ease-out lg:static lg:translate-x-0 ${
          mobileOpen ? "translate-x-0" : "-translate-x-full"
        }`}
      >
        <div className="flex items-center gap-2.5 px-5 py-6">
          <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-emerald-400 to-emerald-600 text-sm font-bold text-slate-950 shadow-lg shadow-emerald-500/20">
            M
          </div>
          <div>
            <p className="text-sm font-semibold tracking-tight text-white">Farmácia MAS</p>
            <p className="text-xs text-slate-500">Painel de gestão</p>
          </div>
          <button
            onClick={onCloseMobile}
            aria-label="Fechar menu"
            className="ml-auto rounded-md p-1.5 text-slate-500 hover:bg-white/[0.06] hover:text-slate-200 lg:hidden"
          >
            <svg
              viewBox="0 0 24 24"
              fill="none"
                  ...
                  return (
                    <li key={item.href}>
                      ...
                      >
                        ...
                        <Icon
                          name={item.icon}
                          className={`h-4 w-4 shrink-0 ${active ? "text-emerald-400" : "text-slate-500 group-hover:text-slate-300"}`}
                        />
                        {item.label}
                      </Link>
                    </li>
        ...
        <div className="border-t border-white/[0.06] px-3 py-3">
          <button
            onClick={sair}
            className="flex w-full items-center gap-2.5 rounded-lg px-3 py-2 text-sm text-slate-400 transition-colors hover:bg-white/[0.04] hover:text-slate-100"
          >
            <svg
              viewBox="0 0 24 24"
              fill="none"
