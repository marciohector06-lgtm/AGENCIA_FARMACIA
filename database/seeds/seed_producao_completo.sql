-- Seed de produção completo, em um único script — junta seed_demo_mip.sql
-- (15 produtos iniciais) + seed_farmacia_completa.sql (86 produtos
-- adicionais) numa passada só, sem depender de nenhum dos dois ter rodado
-- antes. Script manual (SQL Editor do Supabase), fora da cadeia versionada
-- do Alembic. Idempotente por PK: todo INSERT usa UUID fixo + ON CONFLICT
-- (id) DO NOTHING — rodar de novo nunca duplica nem falha.
--
-- Motivo deste arquivo existir: seed_farmacia_completa.sql tinha uma trava
-- (RAISE EXCEPTION) que exigia os 15 produtos de seed_demo_mip.sql já
-- existirem — no Supabase de produção eles nunca tinham sido aplicados, e a
-- trava bloqueava o script inteiro. Aqui os dois conjuntos de dados vêm
-- juntos, na ordem certa (base antes do que depende dela), sem trava
-- nenhuma além da filial precisar existir de antemão.
--
-- ==============================================================================
-- DESVIOS DO PEDIDO ORIGINAL (herdados de seed_farmacia_completa.sql) — leia
-- antes de rodar
-- ==============================================================================
...
--    Nenhum desses vira produto. Os princípios ativos correspondentes ainda
--    são cadastrados — servem pra checar interação/restrição quando um
--    cliente relata usar um remédio controlado, mesmo que a farmácia não venda.
-- 2) 8 princípios ativos extras, só-referência, nunca viram produto —
--    necessários pro FK das interações fechar: Varfarina, Álcool etílico,
--    Ferro, Clopidogrel, 3 classes terapêuticas, Probióticos.
-- 3) Produtos combinados ligados a UM princípio ativo primário (schema não
--    suporta múltiplos ativos por produto).
-- 4) Algumas restrições pedidas (úlcera péptica, cardiopatia, Parkinson,
--    alcoolismo) não têm valor correspondente no enum do banco — de fora.
-- ==============================================================================

-- Única trava mantida: a filial precisa existir de antemão (nenhum dos dois
-- seeds cria filial). Isto NÃO é a trava que causava o bloqueio — essa
-- (checar os 15 produtos anteriores) foi removida, ver acima.
DO $$ BEGIN
...
-- ==============================================================================
-- PARTE 1 — conteúdo de seed_demo_mip.sql (15 produtos iniciais)
-- ==============================================================================
...
-- Princípios ativos (10, todos MIP). Ranitidina sem produto associado —
-- substância retirada do mercado global (contaminação por NDMA, ~2020),
-- ANVISA suspendeu registros.
INSERT INTO principios_ativos (id, nome, classe_terapeutica) VALUES
  ...
  ('c0000000-0000-0000-0000-000000000007', 'Ranitidina', 'Antagonista H2 (uso descontinuado)'),
...
-- Produtos (15, todos tarja='isento'). Nomes comerciais reais só onde o
-- status MIP é bem estabelecido; fabricante-marca é atribuição de demo, não
-- cadastro real (Tylenol/Advil/Neosaldina/Buscopan não são EMS/Medley/
-- Eurofarma de verdade).
...
-- Lotes (1 por produto) — validade sempre relativa a CURRENT_DATE.
...
-- Estoque (1 registro por lote, quantidade_atual=100)
...
-- ==============================================================================
-- PARTE 2 — conteúdo de seed_farmacia_completa.sql (86 produtos adicionais)
-- Nota: NÃO tem mais a trava que checava os 15 produtos da Parte 1 — a Parte
-- 1 acima já garante que eles existem, sempre, na mesma execução.
-- ==============================================================================
...
-- Princípios ativos — 20 pedidos (5 são tarja vermelha na vida real e ficam
-- sem produto associado, ver nota 1 no topo do arquivo).
...
-- Interações medicamentosas (15 pares únicos; ordem canônica a_id < b_id)
...
-- Restrições de uso (20). Só usa os 8 valores existentes de
-- tipo_restricao_enum — pedidos sem valor de enum correspondente ficaram de
-- fora (ver nota 4 no topo do arquivo).
...
-- ==============================================================================
-- Conferência final — totais esperados: 13 / 38 / 101 / 15 / 20 / 101 / 101
-- ==============================================================================
SELECT
